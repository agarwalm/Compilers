...
#	print " "
	...
	print 'declare i32 @createList(i32)'
	print 'declare i32 @insertList(i32*,i32,i32)'
...
def boxingPass(n):
	...
	#list stuff
		...
		for node in n.nodes:
			node=boxingPass(node)
		n=Tag(n,"list")
		return n
	elif isinstance(n, Subscript):
		n.subs=boxingPass(n.subs)
		print n.subs
		return n
...
def flattenStmt(n):
	...
	if isinstance(n, Assign):
		...
		if isinstance(n.expr, Name):
			...
			flatStmts.append(n)
		elif isinstance(n.expr, Tag) and isinstance(n.expr.node, List):
			print n.name
...
def flattenExp(n, x):
	...
	if isinstance(n,Const):
		...
		if isinstance(n.node, Const):
			...
			return x
		#list stuff
		elif isinstance(n.node, List):
			a=[]
			for exp in n.node.nodes:
				var=genSym()
				flv=flattenExp(exp,var)
				a.append(Name(var))
			n.node.nodes=a
			#n=flattenExp(n.nodes)
			temp=Assign(AssName(x),n)
			flatStmts.append(temp)
			print x
			return x
...
def astToLLVM(ast, x):
	...
	#list stuff
		...
		# output_store(str(size),a)
		print "	 "+a+" = call i32* @createList(i32 "+ str(size) +")"
		i=0
		for n in ast.nodes:
			b=genSym()
			print "	 "+b+" = call i32* @insertList(i32* "+a+", i32 "+ str(i) +", i32* "+ n.name +")"
			i+=1
		
		#codegen_augassign(ast, x, op)

		output_store(x,a)

		# f=CallFunc(Name("createList"), [Name("i32 "+a)])
		# v=genSym()
		# codegen_callfunc(f,v)
		return ast.nodes
	elif isinstance(ast,Subscript):
		a=genSymFromVar(ast.expr)
		b=genSym()
		print " "+b+ "= call i32* @getElementFromList(i32*"+ a +", i32*"+ast.subs +" ) " 
		return ast
