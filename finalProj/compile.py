...
def closureConversion(n):
	if isinstance(n, Module):
		...
		if isinstance(n.expr, CallFunc):
			assignedFuncCalls.append(n.name.name)
...
#		
		if ast.flag == "callVar":
			...
			if (tempName not in functionToClosure.keys()):
				if (ast.flag !=""):
					if (genSymFromVar(ast.node.name) not in envToArgs[ast.flag]) or (genSymFromVar(ast.node.name) not in envToBoundedVars[ast.flag]) or (ast.node.name not in assignedFuncCalls):
							sys.exit('ERROR! Undefined Function')
			
			if(ast.flag != "") and (genSymFromVar(ast.node.name) in envToArgs[ast.flag]):
					astToLLVM(CallFunc(Name(genSymFromVar(ast.node.name)), ast.args, "callVar"), x)
				
			elif ast.node.name in assignedFuncCalls:
					astToLLVM(CallFunc(Name(genSymFromVar(ast.node.name)), ast.args, "callVar"), x)
...
def codegen_freeVar_callfunc(ast,x):
	...
	if ast.flag == "callVar":
		...
		print "   "+c+" = bitcast i8* "+a+" to "+functype
