...
def astToLLVM(ast, x):
	...
	if isinstance(ast, Assign):
		...
		return codegen_bitwise(ast,x, "or")

	elif isinstance(ast, Bitand):
		return codegen_bitwise(ast,x, "and")

	elif isinstance(ast, Bitxor):
		return codegen_bitwise(ast,x, "xor")
...
def codegen_assign_const(ast):
	output_store(str(ast.expr.value),ast.nodes[0].name)

def codegen_assign_name(ast):
	a = genSym()
	output_load(a, ast.expr.name);
	output_store(a, ast.nodes[0].name)
